...
<c:if test="${fn:length(recibos) > 0}">
    <table width="95%" class="grid">
        <thead>
            <tr>
                <th >Cod.</th><th >Data Requisição</th><th >Data Entrega</th><th >Consumidor</th><th >Ação</th>
            </tr>
        </thead>
        <tbody>
            <c:forEach var="recibo" items="${recibos}">
                <tr>
                    ...
                    <td>
	        	<a href="requisicao.get.logic?requisicao.id=${requisicao.id}"><img src="images/edit.png" title="Editar requisição"/></a>

                        <input type="button" class="button" value="Emitir" onclick="emitirRecibo(${recibo.id})"/></td>
                </tr>
